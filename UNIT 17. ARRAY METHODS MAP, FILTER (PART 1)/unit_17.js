...
function t2() {
  let out = a2.map((item) => a2_res.push(item ** 2));
...
function t3() {
  let out = a3.map((item) => {
    if (typeof item == 'number') a3_res.push(item);
  });
...
function t4() {
  let out = a4.forEach((element) => {
    if (typeof element == 'number') a4_res.push(element);
  });
...
function t5() {
  let out = b1.filter((elem) => {
    ...
    return true; //в данной задачее работает и без  return true
  });
...
function t9() {
  let out = b9.filter((elem) => {
    ...
    else b9_string.push(elem);
    return true;
  });
  console.log(b9_num);
  console.log(b9_string);
  document.querySelector('.out-9').innerHTML = b9_num + '=======';
  document.querySelector('.out-9').innerHTML += b9_string;
}
...
function t10() {
  let out = b10.filter((elem, index) => {
    console.log(elem);
    if (elem.indexOf(3) != -1) b10_res.push(elem);
    return true;
  });
