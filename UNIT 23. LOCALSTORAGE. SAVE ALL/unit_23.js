...
function saveCartToLS(save) {
  //функция t10
  localStorage.setItem('card', JSON.stringify(save));
  showCart();
}
...
function plusProduct() {
  ...
  document.querySelectorAll('.control').forEach(function (elem, index) {
    ...
    elem.querySelector('.plus').onclick = function () {
      ...
      let dataP = this.getAttribute('data-art');

      pickUp[dataP]++;
      saveCartToLS(pickUp); //сохраняем в LS (увлечения) плюса на один при нажатие кнопки
    };
    ...
    elem.querySelector('.minus').onclick = function () {
      ...
      saveCartToLS(pickUp); //сохраняем в LS (уменьшения) минуса на один при нажатие кнопки
    };
    totalQuantity();
  });
}
...
function totalQuantity() {
  // функция t13  выводим общие количество товара
  let sum = 0;
...
function checkCart() {
  // функция t14  проверяем содержит ли карниза что то после загрузки страницы
...
window.addEventListener('storage', checkCart); ///этот слушатель нужен для того, чтобы все страницы общего домена получали доступ к
